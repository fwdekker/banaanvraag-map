...
As of Azgaar v1.108.12, using Firefox is preferred over Chromium because the `Release` export is considerably faster.
...
Note that, SVGs exported from Azgaar (as of v1.108.12) look less pretty when imported in Inkscape (as of v1.4), most notably due to strangely aligned nation labels.

#### PNG
To export the entire map as PNG:
1. Click `Export`, select `tiles`, set `Columns` to `2`, `Rows` to `2`, and `Scale` to `1`, and click `Download`.
2. Extract the downloaded `.zip`, delete `schema.png`, and join the images using ImageMagick command
   ```shell
   montage *.png -geometry +0 banaanvraag.png
   ```
